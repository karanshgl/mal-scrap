...
class UrlFilter(SearchFilter):
    def filter_queryset(self, request, queryset, view):
        ...
        log.debug("Searching for \"{}\" in: {}".format(search_terms[0], search_fields))
        term = unicodedata.normalize('NFKD', search_terms[0]).encode('ascii','ignore')
        value = '%{0}%'.format(term)
        return queryset.filter(getattr(view.model, 'url').like(value))
...
class MalwareUrlViewSet(ViperGenericViewSet):
    """List and Retrieve Malware instances"""
    model = Malware
    serializer_class = ObjectSerializer
    lookup_field = 'sha256'

    filter_backends = (UrlFilter,FieldsFilter)
    ordering_fields = ('id',)  # '__all__' does not work!
    search_fields = ('url')
    ...
    def list(self, request, *args, **kwargs):
        """List all Malware instances"""
        return super(MalwareUrlViewSet, self).list(request, *args, **kwargs)
...
class ChartDotAPIViewSet(ViperGenericViewSet):
    """List and Retrieve Malware instances"""
    model = Malware
    serializer_class = ObjectSerializer

    @get_project_open_db()
    def list(self, request, project=None, db=None, *args, **kwargs):
        """List all Malware instances"""
        session = db.Session();
        obj = session.query(self.model).all()
        domain = []
        dot = []
        for o in obj:
            website = o.url.split('/')[2]
            loc = website.split('.')[-1]
            domain.append(website)
            dot.append(loc)

        dot_count = Counter(dot).most_common(10)
        #print(dot_count)
        #labels = dot_count.keys()
        #default_items = dot_count.values()
        labels = []
        default_items = []
        for val in dot_count:
            labels.append(val[0])
            default_items.append(val[1])
        data = {
                "labels": labels,
...
class ChartDomAPIViewSet(ViperGenericViewSet):
    """List and Retrieve Malware instances"""
    model = Malware
    serializer_class = ObjectSerializer
    ...
    def list(self, request, project=None, db=None, *args, **kwargs):
        ...
        dot_count = Counter(domain).most_common(10)
        #print(dot_count)
        #labels = dot_count.keys()
        #default_items = dot_count.values()
        labels = []
