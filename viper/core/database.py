...
class Database:
    ...
    def add(self, obj, name=None, tags=None, url=None, parent_sha=None, notes_body=None, notes_title=None):
        session = self.Session()
        if not name:
            name = obj.name
    ...
    def find(self, key, value=None, offset=0):
        ...
        if key == 'all':
            ...
            rows = session.query(Malware).filter(Malware.name.startswith(prefix_val) |
                                                 ...
                                                 Malware.url.startswith(prefix_val) |
            ...
            rows = session.query(Malware).filter(Malware.url.like(value)).all()
        else:
